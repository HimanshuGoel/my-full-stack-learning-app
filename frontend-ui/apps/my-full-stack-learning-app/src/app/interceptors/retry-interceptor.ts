...
export const retryInterceptor: HttpInterceptorFn = (
  ...
  const retryCount =
    req.context.get(COFFEE_API_RETRY_COUNT) || environment.retryInterceptorRetryCount;
  ...
  return next(req).pipe(
    retry({
      ...
      delay: (err: unknown, attemptNum: number) => {
        console.error(
          `[RetryInterceptor] => Encountered an error while retrying request on attempt ${attemptNum}: `,
          err
        );
